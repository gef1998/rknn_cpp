...
void STrack::activate(byte_kalman::KalmanFilter &kalman_filter, int frame_id)
{
    ...
    this->mean = mc.first;        // [cx, cy, a, h, 0, 0, 0, 0]
    this->covariance = mc.second; // 初始协方差矩阵
	...
	//frame_id == 1：第一帧的轨迹直接激活
	//frame_id > 1：新轨迹需要在后续帧匹配成功才激活（防止误检）
	if (frame_id == 1)
	{
