...
public:
    ...
    int tryGet(outputType &outputData, int timeout_ms);
...
template <typename rknnModel, typename inputType, typename outputType>
int rknnPool<rknnModel, inputType, outputType>::tryGet(outputType &outputData, int timeout_ms) {
    std::lock_guard<std::mutex> lock(queueMtx);    
    // 如果队列为空，立即返回
    if (futs.empty()) {
        return 1;
    }    
    auto status = futs.front().wait_for(std::chrono::milliseconds(timeout_ms));
