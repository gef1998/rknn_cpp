...
// =============init encoder========================
  unsigned char* input_data[io_num_encoder.n_input];
  int            input_type[io_num_encoder.n_input];
  int            input_layout[io_num_encoder.n_input];
  int            input_size[io_num_encoder.n_input];

  input_data[0]   = NULL;
  input_type[0]   = RKNN_TENSOR_UINT8;
  input_layout[0] = RKNN_TENSOR_NHWC;
  input_size[0]   = input_attrs_encoder[0].size;
  ...
  for (int i = 0; i < io_num_encoder.n_input; i++) {
    input_data[i] = new unsigned char[input_attrs_encoder[i].size];

    printf("%s\n", encoder_input_paths_split[i].c_str());

    FILE* fp = fopen(encoder_input_paths_split[i].c_str(), "rb");
    if (fp == NULL) {
      perror("open failed!");
      return -1;
    }

    fread(input_data[i], input_attrs_encoder[i].size, 1, fp);
    fclose(fp);
    if (!input_data[i]) {
      return -1;
    }
  ...
  for (int i = 0; i < io_num_encoder.n_input; i++) {
    inputs_encoder[i].index        = i;
    inputs_encoder[i].pass_through = 0;
    inputs_encoder[i].type         = (rknn_tensor_type)input_type[i];
    inputs_encoder[i].fmt          = (rknn_tensor_format)input_layout[i];
    inputs_encoder[i].buf          = input_data[i];
    inputs_encoder[i].size         = input_size[i];
  }
...
// =============init preprosess========================
  ...
  output_attrs_preprosess[0].fmt = RKNN_TENSOR_NCHW;
...
// =============init grid sample========================
  ...
  rknn_tensor_mem* input_mems_grid_sample[io_num_grid_sample.n_input];
  ...
  ret = rknn_set_io_mem(ctx_grid_sample, output_mems_preprosess[1], &input_attrs_grid_sample[1]);
  ...
  output_attrs_grid_sample[0].fmt = RKNN_TENSOR_NCHW;  
  ...
  // =============init decoder========================
  unsigned char* input_data_decoder[io_num_decoder.n_input];
  int            input_type_decoder[io_num_decoder.n_input];
  int            input_layout_decoder[io_num_decoder.n_input];
  int            input_size_decoder[io_num_decoder.n_input];

  input_data_decoder[0]   = NULL;
  input_type_decoder[0]   = RKNN_TENSOR_FLOAT16;
  input_layout_decoder[0] = RKNN_TENSOR_NCHW;
  input_size_decoder[0]   = input_attrs_decoder[0].size * 2;

  input_data_decoder[1]   = NULL;
  input_type_decoder[1]   = RKNN_TENSOR_FLOAT16;
  input_layout_decoder[1] = RKNN_TENSOR_NCHW;
  input_size_decoder[1]   = input_attrs_decoder[1].size * 2;

  rknn_input inputs_decoder[io_num_decoder.n_input];
  memset(inputs_decoder, 0, io_num_decoder.n_input * sizeof(rknn_input));

  inputs_decoder[0].index        = 0;
  inputs_decoder[0].pass_through = 1;  // Pass through mode
  inputs_decoder[0].type         = (rknn_tensor_type)input_type_decoder[0];
  inputs_decoder[0].fmt          = (rknn_tensor_format)input_layout_decoder[0];
  inputs_decoder[0].buf          = output_mems_grid_sample[0]->virt_addr;
  inputs_decoder[0].size         = input_size_decoder[0];

  inputs_decoder[1].index        = 1;
  inputs_decoder[1].pass_through = 1; // Pass through mode
  inputs_decoder[1].type         = (rknn_tensor_type)input_type_decoder[1];
  inputs_decoder[1].fmt          = (rknn_tensor_format)input_layout_decoder[1];
  inputs_decoder[1].buf          = output_mems_preprosess[0]->virt_addr;
  inputs_decoder[1].size         = input_size_decoder[1];

  // Set input
  ret = rknn_inputs_set(ctx_decoder, io_num_decoder.n_input, inputs_decoder);
  if (ret < 0) {
    printf("rknn_input_set fail! ret=%d\n", ret);
    return -1;
  }
  ...
  pthread_create(&tid1, NULL, thread_func, &ctx_encoder);
  pthread_create(&tid2, NULL, thread_func, &ctx_preprosess);
