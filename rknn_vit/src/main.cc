...
-------------------------------------------*/
// Sigmoid函数实现
std::string formatSigmoid(float x) {
  float sig_val = 1.0f / (1.0f + exp(-x));
...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < io_num.n_output; i++) {
    ...
    LabelLoader imagenet_label = LabelLoader("/home/jz/catkin_ws/src/rknn_cpp/rknn_vit/model/imagenet.json");

    // 设置文本参数
    int fontFace = cv::FONT_HERSHEY_SIMPLEX;
    double fontScale = 0.5;
    cv::Scalar color(0, 255, 0); // 绿色文本
    int thickness = 2;
    int baseline = 0;
    ...
    // 计算文本位置(图像底部)
    cv::Point textOrg(10, orig_img_rgb.rows - 10);
    
    // 添加背景矩形提高可读性
    cv::rectangle(orig_img_rgb, 
                 textOrg + cv::Point(0, baseline),
                 textOrg + cv::Point(textSize.width, -textSize.height),
                 cv::Scalar(0, 0, 0), cv::FILLED);
    
    // 绘制文本
    cv::putText(orig_img_rgb, text, textOrg, 
               fontFace, fontScale, color, thickness);

    // 显示图像
    cv::imshow("Classification Result", orig_img_rgb);
    cv::waitKey(0);
